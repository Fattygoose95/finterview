...
def main():
    ...
    # 保存修改
    save = input("\nSave changes to questions.js? (y/n): ").lower().strip()
    if save == 'y':
        save_questions(questions_file, updated_questions, prefix, suffix)
        
        # 创建下一步计划
        print("\n=== NEXT STEPS ===")
        print("1. Risk Management repair: COMPLETED (77 questions)")
        print("2. Other categories: Detailed answers updated to Finance Bro prompt")
        print("3. Remaining work:")
        
        total_remaining = sum(needs_fix_by_role.values())
        if total_remaining > 0:
            print(f"   - Generate concise answers for {total_remaining} questions in other categories")
            for role, count in needs_fix_by_role.items():
                if count > 0:
                    print(f"     * {role.upper()}: {count} questions")
        
        print("\n4. Test Finance Bro interaction in industry-practice.html")
    else:
